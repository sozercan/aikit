...
func installExllama(c *config.InferenceConfig, s llb.State, merge llb.State) llb.State {
	...
	savedState := s
	s = s.Run(utils.Sh("apt-get update && apt-get install --no-install-recommends -y git ca-certificates python3-pip python3-dev make g++ && curl -LsSf https://astral.sh/uv/install.sh | sh && apt-get clean"), llb.IgnoreCache).Root()
