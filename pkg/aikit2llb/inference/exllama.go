...
func installExllama(c *config.InferenceConfig, s llb.State, merge llb.State) llb.State {
	...
	s = s.Run(utils.Shf("set -x && cd /tmp/localai/backend/python/%[1]s && rm -rf .git && . $HOME/.cargo/env && python3 -m grpc_tools.protoc -I../.. --python_out=. --grpc_python_out=. backend.proto && export BUILD_TYPE=cublas && ./install.sh", backend)).Root()
