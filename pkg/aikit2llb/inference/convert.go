...
func installOpenCV(s llb.State, merge llb.State) llb.State {
	...
	s = s.Run(utils.Shf("apt-get update && mkdir -p /tmp/generated/images && apt-get install -y libopencv-imgcodecs406 libgomp1 libdap27 libdapclient6v5 && apt-get clean"), llb.IgnoreCache).Root()
	diff := llb.Diff(savedState, s)
	merge = llb.Merge([]llb.State{merge, diff})

	sdURL := "https://sertaccdn.azureedge.net/localai/test/stablediffusion"
	var opts []llb.HTTPOption
